...
class Centinela {
	...
	function Centinela($auto = TRUE) { //true por defecto cuando se crea y supone ya se esta logueadoen el sistema
	...
	function verifyDeveloper($isFirst = FALSE, $username = "", $password = "") {
		...
		if ($isFirst == TRUE) { //si no esta registrado aun
			/*el orden de las verificaciones es obligado, 
			 * porque si ponia por ejemplo primero insertar en user_session a pues 
			 * si esataba la sesion aun sin vencerse de todas formas agregaba primero a user_sessions, 
			 * lo cual es un error gravisimo ahora verificar sesion concurrente*/
			$flag1 = true;
			...
			/*el id es la llave de la tabla user_session, pues como vimos aunque las sesiones sean unicas 
			 * son regeneradas cada x tiempo, y habria problemas en el update session cuando expiren las sesiones, 
			 * ya no podrian cogerse  el old-session_id*/
	...
	function login($isFirst = FALSE, $username = "", $password = "", &$message) {
		//isfirst se llama con true solo al loguerase desde el controlador sys/system en la funcion login
		
		
		$datenow = $this->today();
		
		if ($username == 'developer') {
			return $this->verifyDeveloper($isFirst, $username, $password);
		}
