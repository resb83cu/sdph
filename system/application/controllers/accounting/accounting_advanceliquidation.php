...
class Accounting_advanceliquidation extends Controller
{
    ...
    function showLiquidationLateView()
    {
        $centinela = new Centinela ();
        $flag = $centinela->accessTo('accounting/accounting_advanceliquidation');
        if ($flag) {
            $this->load->view('sys/header_view');
            $this->load->view('accounting/accounting_liquidationlate_view');
            $this->load->view('sys/footer_view');
        } else {
            $this->redirectError();
        }
    ...
    function getAdvanceLiquidationRequests()
    {
        ...
        if ($flag) {
            ...
            return $this->conn->getAdvanceLiquidationRequests($dateStart, $dateEnd);
    ...
    function getAdvanceLiquidationRequestsGroup()
    {
        ...
        if ($flag) {
            ...
            return $this->conn->getAdvanceLiquidationRequestsGroup($dateStart, $dateEnd, $province, $center);
            //echo "<pre>"; print_r($data); echo "</pre>";
        } else {
            $this->redirectError();
        }
...
//            echo "<pre>"; print_r($liq); echo "</pre>";
    ...
    function manageAdvanceLiquidationGroup($request_ids)
    {
        ...
        $fecha = new Dates ();
        $data = $this->conn->getAdvanceLiquidationById($request_ids[0]);//paso el request_id y obtengo los datos d esta solicitud
        $this->load->model('conf/conf_costcenters_model', 'costcenter', true);
        if ($data->advance_requested === "f") {//si no se ha solicitado el anticipo
            if ($data->center_idadvance == 47) {
                $this->costcenter->incrementConsecutive(40);
                $consecutive = $this->costcenter->getCenterById(40)->center_consecutive;
            } else {
...
//        }
		if ($data->diet_entrancedate == '2017-03-12') {
            $diasHospedaje++;
        }
        $pagoDinero = 6.5;
...
//            echo "<pre>"; print_r($liq); echo "</pre>";
        ...
        $html = '<table width="700" border="1">
                    ...
                    <tr>
                      <td class="Pdf"><div align="center">A. Pagar.</div></td>
                      <td class="Pdf">&nbsp;</td>
                      <td colspan="2">&nbsp;</td>
                      <td class="Pdf">&nbsp;</td>
                      <td class="Pdf">&nbsp;</td>
    ...
    function reportControlAdvanceLiquidation($center)
    {
        if ($this->session->userdata('person_id') == 0) {
            return "{success: false, errors: { reason: 'Su sesi&oacute;n ha expirado. Ingrese de nuevo al sistema.' }, prorogation: 'no'}";
        }
    ...
    function reportAdvanceLiquidationByResponsable($ids)
    {
        ...
        $workers = $this->getRequestDataFromAdvance($requests);//esto es para pintar en la 2da tabla los trabajadores el estimado, etc.
...
//            }
			if ($request->diet_entrancedate == '2017-03-12') {
				$diasHospedaje++;
			}
            $value = array(
    ...
    function getLiquidationLateGrid()
    {
        $date = new Dates();
        $now = $date->nowShort();
        return $this->liquidation->getLiquidationLateGrid($now);
    }
