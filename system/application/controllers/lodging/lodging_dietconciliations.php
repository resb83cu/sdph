...
class Lodging_dietconciliations extends Controller {
	...
	log_message('error', $this->session->userdata('roll_id'), true);
        if ($this->session->userdata('roll_id') >= 4) {
		  ...
		  log_message('error', 'Entre al metodo.', true);
		...
		log_message('error', 'Antes del metodo', true);
    ...
    public function setPdfCociliation($array) {
        if ($this->session->userdata('roll_id') >= 4) {
            ...
            if ($cant > 0) {
                ...
                $length = count($request_ids);
                for ($i = 0; $i < $length; $i++) {
                    $this->conn->setPay($request_ids[$i]);
                }
            }
