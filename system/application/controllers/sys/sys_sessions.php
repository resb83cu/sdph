 ...
 * @orm sys_logs
 */
class Sys_sessions extends Controller {
	
	function __construct() {
		parent::Controller ();
		$this->load->model ( 'sys/sys_sessions_model', 'conn', true );
	}
	
	function index() {
		$centinela = new Centinela ( );
		$flag = $centinela->accessTo ( 'sys/sys_sessions' );
		if ($flag) {
			$this->load->view ( 'sys/header_view' );
			$this->load->view ( 'sys/sys_sessions_view' );
