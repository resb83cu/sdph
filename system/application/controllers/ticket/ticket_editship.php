 ...
 */
class Ticket_editship extends Controller {
	
	function __construct() {
		parent::Controller ();
		$this->load->model ( 'ticket/ticket_editship_model', 'conn', true );
	
	}
	
	function index() {
		$centinela = new Centinela ( );
		$flag = $centinela->accessTo ( 'ticket/ticket_editship' );
		if ($flag) {
			$this->load->view ( 'sys/header_view' );
			$this->load->view ( 'ticket/ticket_editship_view' );
	...
	public function setDataGrid() {
		...
		return $this->conn2->getDataShip ( $to, $from, $dateStart, $dateEnd, $motive );
	}
	...
	function shipPdf($request_id, $ticket_date) {
		$centinela = new Centinela ( );
		$flag = $centinela->accessTo ( 'ticket/ticket_editship' );
		if ($flag) {
			$data = $this->conn->getById ( $request_id, $ticket_date );
