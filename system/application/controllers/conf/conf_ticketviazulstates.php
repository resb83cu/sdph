 ...
 * @orm conf_transports
 */
class Conf_ticketviazulstates extends Controller {
	
	function __construct() {
		parent::Controller ();
		$this->load->model ( 'conf/conf_ticketviazulstates_model', 'conn', true );
	}
	
	function index() {
		$centinela = new Centinela ( );
		$flag = $centinela->accessTo ( 'conf/conf_ticketviazulstates' );
		if ($flag) {
			$this->load->view ( 'sys/header_view' );
			$this->load->view ( 'conf/conf_ticketviazulstates_view' );
	...
	function getById($viazulstate_id) {
		$data = $this->conn->getById ( $viazulstate_id );
		die ( "{data : " . json_encode ( $data ) . "}" );
	}
