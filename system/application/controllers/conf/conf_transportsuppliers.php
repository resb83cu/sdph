 ...
 * @orm conf_transports
 */
class Conf_transportsuppliers extends Controller {
	
	function __construct() {
		parent::Controller ();
		$this->load->model ( 'conf/conf_transportsuppliers_model', 'conn', true );
	}
	
	function index() {
		$centinela = new Centinela ( );
		$flag = $centinela->accessTo ( 'conf/conf_transportsuppliers' );
		if ($flag) {
			$this->load->view ( 'sys/header_view' );
			$this->load->view ( 'conf/conf_transportsuppliers_view' );
	...
	function getById($supplier_id) {
		$data = $this->conn->getById ( $supplier_id );
		die ( "{data : " . json_encode ( $data ) . "}" );
	}
