...
Ext.onReady(function() {
	...
	var p = new Ext.Panel({
        title: 'Configuraci&oacute;n -> Tipo de Transporte de Pasaje',
        collapsible:false,
    ...
    function update_ventana(id){
	    ...
	    var updateWindow;
		 ...
		 /*
	     * Creamos el formulario de alta/modificación
	     */
	    ...
	    Transports.Form.addButton({
	        ...
	        handler : function() {
	            Transports.Form.getForm().submit({
	                url : baseUrl+'index.php/conf/conf_tickettransports/insert',
	  	...
	  	var title = 'Agregar';
		if (id > 0){
        	Transports.Form.load({url:baseUrl+'index.php/conf/conf_tickettransports/getById/'+id});
        	title = 'Editar';
		}
    ...
    function delRecords(btn) {
	    if (btn == 'yes') {
			for (var i = 0, len = array.length; i < len; i++) {
		        Ext.Ajax.request({
				   url: baseUrl+'index.php/conf/conf_tickettransports/delete/'+array[i].get('transport_id'),
				   method: 'GET',
