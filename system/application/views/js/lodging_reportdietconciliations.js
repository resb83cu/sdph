...
var dataReaderCafeteria = new Ext.data.JsonReader({
    root:'data'
},dataRecordCafeteria);
...
Ext.onReady(function() {
    ...
    var p = new Ext.Panel({
        title: 'Reportes -> Reporte Conciliacion Dieta',
        collapsible:false,
    ...
    Conciliations.conciliationsDataProxy = new Ext.data.HttpProxy({
        url: baseUrl+'index.php/lodging/lodging_dietconciliations/setDataCociliation/true/no',
        method: 'POST'
    });
            ...
            handler: function(){
                if ( conciliationsDataStore.getCount() > 0 ){
                    ...
                    if (count > 0) {
                        ...
                        Conciliations.filterForm.getForm().getEl().dom.action = baseUrl+'index.php/lodging/lodging_dietconciliations/setPdfCociliation/'+conciliados;
                        Conciliations.filterForm.getForm().getEl().dom.method = 'POST';
                        Conciliations.filterForm.getForm().submit();
                    } else {
                        Conciliations.filterForm.getForm().getEl().dom.action = baseUrl+'index.php/lodging/lodging_dietconciliations/setDataCociliation/true/si';
                        Conciliations.filterForm.getForm().getEl().dom.method = 'POST';
                        Conciliations.filterForm.getForm().submit();
                    }
                }
        ...
        items:[{
            ...
            items:[{
                ...
                items:[	new Ext.form.ComboBox({
                    ...
                    mode: 'local',
                    triggerAction: 'all', 					
                    emptyText: 'Seleccione Cafeteria...',
                    selectOnFocus: true,
                    width: 200,
                    id: 'filter_cafeteria_id',
                    name : 'filter_cafeteria_id',
                    listeners: {
                        'blur': function(){
                            var flag = dataStoreCafeteria.findExact( 'cafeteria_id', Ext.getCmp('filter_cafeteria_id').getValue());
                            if (flag == -1){
                                Ext.Msg.alert('Valor Inv&aacute;lido', 'Debe seleccionar un valor de la lista y no introducir un dato err&oacute;neo.');
                                Ext.getCmp('filter_cafeteria_id').reset();
                                return false;
                            }
                        }
                    }
    ...
    */
    Conciliations.filterForm.addButton({
        ...
        handler : function() {
            ...
            var hotel = Conciliations.filterForm.findById('filter_cafeteria_id').getValue();
