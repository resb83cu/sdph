...
Ext.onReady(function () {
    ...
    var p = new Ext.Panel({
        title: 'Contabilidad -> Solicitud Anticipo para Grupo',
        collapsible: false,
    ...
    var miboton = new Ext.Button({
        text: 'Generar anticipo para Grupo',
        disabled: true,
        handler: function () {
            array = sm2.getSelections();
//            Ext.MessageBox.alert('Error', JSON.stringify(dataStoreAdvanceLiquidation.collect('person_groupresponsable')));
            var ids = '';
            var len = array.length;
            if (len === 0) {
                Ext.MessageBox.alert('Error', 'Debe seleccionar una solicitud.');
            } else if (len > 0) {
                //verificar que todas las solicitudes seleccionadas tengan un responsable
                for (var i = 0; i < len; i++) {
                    if (array[i].get('person_groupresponsable') === '') {
                        Ext.MessageBox.alert('Error', 'Asegurese que las solicitudes seleccionados tengan asignado un Responsable.');
                        sm2.clearSelections();
                        return false;
                    }
                }
                if (len > 0) {
                    var resp = array[0].get('person_groupresponsable');
                    for (var i = 1; i < len; i++) {
                        if (resp !== array[i].get('person_groupresponsable')) {
                            Ext.MessageBox.alert('Error', 'El Responsable no coincide en todas las solicitudes.\n Asegurese que las solicitudes seleccionadas tengan asignado el mismo responsable');
                            sm2.clearSelections();
                            return false;
                        }
                    }
                ...
                /*var resp = array[0].get('person_groupresponsable');
                var childs = dataStoreAdvanceLiquidation.query('person_groupresponsable', resp).getCount();
                if (len < childs) {
                    Ext.MessageBox.alert('Error', 'Por favor seleccione todos los anticipos asignados a ' + resp + '\n Usted seleccion&oacute ' + len + ' y debe seleccionar ' + childs);
                    sm2.clearSelections();
                    return false;
                }*/
                for (var i = 0; i < len; i++) {
                    if (i > 0) {
                        ids += '-' + array[i].get('request_id');
                    } else {
                        ids = array[i].get('request_id');
                    }
                }

                Advance.filterForm.getForm().getEl().dom.action = baseUrl + 'index.php/accounting/accounting_advanceliquidation/reportAdvanceLiquidationByResponsable/' + ids;
    ...
    Advance.dataProxyAdvanceLiquidation = new Ext.data.HttpProxy({
        url: baseUrl + 'index.php/accounting/accounting_advanceliquidation/getAdvanceLiquidationRequestsGroup',
        method: 'POST'
    });
    ...
    dataStoreAdvanceLiquidation = new Ext.data.GroupingStore({
        ...
        sortInfo: {
            field: 'person_groupresponsable',
            direction: "ASC"
        },
        groupField: 'person_groupresponsable'
    });
    ...
    Advance.advanceLiquidationColumnMode = new xg.ColumnModel(
        [new xg.RowNumberer(),
            ...
            id: 'person_groupresponsable',
            name: 'person_groupresponsable',
            header: "Persona Autorizada Grupo",
            width: 120,
            dataIndex: 'person_groupresponsable',
            sortable: false
        }, {
            id: 'request_area',
            name: 'request_area',
