...
Ext.onReady(function() {
    ...
    sm2 = new xg.CheckboxSelectionModel({
        listeners: {
	        selectionchange: function(sm) {
	            if (sm.getCount()) {
	            	Eventual.eventualGrid.pdfButton.enable();
	            } else {
	            	Eventual.eventualGrid.pdfButton.disable();
	            }
	        }
    	}
    });
	...
	var p = new Ext.Panel({
        title: 'Pasaje -> Gestionar pasaje Eventuales',
        collapsible:false,
    ...
    Eventual.eventualRecord = new Ext.data.Record.create([
     ...
     */
    Eventual.eventualGridReader = new Ext.data.JsonReader({
		root: 'data',
        totalProperty: 'count'/*,
        id: 'request_id'*/},
        Eventual.eventualRecord
    );

    /*
     * Creamos el DataProxy para carga remota de los datos
     */
    Eventual.eventualDataProxy = new Ext.data.HttpProxy({
        url: baseUrl+'index.php/ticket/ticket_editeventual/setDataGrid',
        method: 'POST'
    });

    eventualDataStore = new Ext.data.GroupingStore({
        id: 'eventualDS',
        proxy: Eventual.eventualDataProxy,
        reader: Eventual.eventualGridReader,
     ...
     */
    Eventual.eventualColumnMode = new xg.ColumnModel(
       [new xg.RowNumberer(),
       	sm2,
       {
            id: 'request_id',
     ...
     */
    Eventual.eventualGrid = new xg.GridPanel({
        id : 'ctr-eventual-grid',
        store : eventualDataStore,
        cm : Eventual.eventualColumnMode,
        ...
        tbar:[{
		            ...
		            disabled: false,
		            handler: function(){
							if (eventualDataStore.getCount() > 0 ){
								Eventual.filterForm.getForm().getEl().dom.action = baseUrl+'index.php/ticket/ticket_editeventual/eventualPdf';
								Eventual.filterForm.getForm().getEl().dom.method = 'POST';
								Eventual.filterForm.getForm().submit();
		            ...
		            handler: function(){
			            	    ...
			            	    if (array.length > 0 && Ext.getCmp('filter_edit_state_id').getValue() != ''){
									for (var i = 0, len = array.length; i < len; i++) {
								        ...
								        Ext.Ajax.request({
										   url: baseUrl+'index.php/ticket/ticket_editeventual/insertMulti/'+array[i].get('request_id')+'/'+array[i].get('ticket_date')+'/'+Ext.getCmp('filter_edit_state_id').getValue(),
										   method: 'GET',
									...
									var state = Ext.getCmp('filter_edit_state_id').getValue();
						           	eventualDataStore.baseParams = {
										dateStart: startDate.dateFormat('Y-m-d'),
										dateEnd: endDate.dateFormat('Y-m-d'),
										motive: motive,
										state: state
						           	};
						           	eventualDataStore.load({params: {start:0,limit:50}});
						           	Ext.getCmp('filter_edit_state_id').setValue('');
						           	Eventual.eventualGrid.getStore().reload();
			            ...
			            handler: function(){
									...
									if (len > 1){
										...
										Eventual.filterForm.getForm().getEl().dom.action = baseUrl+'index.php/ticket/ticket_editeventual/etecsaPdf/'+id+'/'+date ;
										Eventual.filterForm.getForm().getEl().dom.method = 'POST';
										Eventual.filterForm.getForm().submit();							
									}
			            }
	...
	Eventual.filterForm = new Ext.FormPanel({
        id: 'form-filtro',
        region: 'north',
        split: false,
        frame: true,
	...
	Eventual.filterForm.addButton({
       text : 'Borrar filtro',
       disabled : false,
       formBind: true,
       handler : function() {
           Eventual.filterForm.getForm().reset();
    ...
    */
   	Eventual.filterForm.addButton({
       	...
       	formBind: true,
       	handler : function() {
           	var startDate = Eventual.filterForm.findById('startdt').getValue();
           	var endDate = Eventual.filterForm.findById('enddt').getValue();
			var motive = Eventual.filterForm.findById('filter_motive_id').getValue();
			var state = Ext.getCmp('filter_edit_state_id').getValue();
           	eventualDataStore.baseParams = {
				dateStart: startDate.dateFormat('Y-m-d'),
				dateEnd: endDate.dateFormat('Y-m-d'),
     ...
     */
    Eventual.eventualGrid.on('rowdblclick',function( grid, row, evt) {
        selectedId = eventualDataStore.getAt(row).data.request_id;
        selectedDate = eventualDataStore.getAt(row).data.ticket_date;
    ...
    function update_ventana(id, date){
	    ...
	    updateForm.addButton({
	        ...
	        handler : function() {
	            ...
	            updateForm.getForm().submit({
	                url : baseUrl+'index.php/ticket/ticket_editeventual/insert',
	                waitMsg : 'Salvando datos...',
	                failure: function (form, action) {
	                    Ext.MessageBox.show({
	                        title: 'Error al salvar los datos',
	                    ...
	                    sm2.clearSelections();
	                    eventualDataStore.load({params: {start:0,limit:15}});
	                },
	                success: function (form, request) {
	                    Ext.MessageBox.show({
	                        title: 'Datos salvados correctamente',
	    ...
	    updateForm.load({url:baseUrl+'index.php/ticket/ticket_editeventual/getById/'+id+'/'+date});
     ...
     */
	Eventual.filterForm.render(Ext.get('eventual_grid'));
	Eventual.eventualGrid.render(Ext.get('eventual_grid'));
    //eventualDataStore.load({params: {start:0,limit:30}});
});
    ...
    function filter(btn) {
	    if (btn == 'yes') {
           	...
           	var endDate = Ext.getCmp('enddt').getValue();
			var motive = Ext.getCmp('filter_motive_id').getValue();
			var state = Ext.getCmp('filter_edit_state_idHidden').getValue();
           	eventualDataStore.baseParams = {
				dateStart: startDate.dateFormat('Y-m-d'),
				dateEnd: endDate.dateFormat('Y-m-d'),
				motive: motive,
