...
class User_users_model extends Model
{
    ...
    public function getDataGridContable($to, $from, $name, $province)
    {
        $this->load->model('user/user_users_model');
        $person_persons_table = 'person_persons';
        $this->db->select('user_users.person_id,
        ...
        $this->db->from(self::TABLE_NAME);
//        $this->db->join($person_workers_table, $person_workers_table . '.person_id = ' . self::TABLE_NAME . '.person_id', 'inner');
        $this->db->join($person_persons_table, $person_persons_table . '.person_id = ' . self::TABLE_NAME . '.person_id', 'inner');
        $this->db->join('conf_provinces', 'conf_provinces.province_id = ' . $person_persons_table . '.province_id', 'inner');
        $this->db->limit($to, $from);
        $this->db->where($person_persons_table . '.person_deleted', 'no');
        $this->db->where("(user_users.roll_id = 7 OR user_users.roll_id = 8)");
        if (!empty($province)) {
            $this->db->where($person_persons_table . '.province_id', $province);
        }
        if (!empty($name)) {
            $this->db->like('LOWER(user_users.user_name)', strtolower($name));
        }
        $this->db->order_by('user_users.locked asc, person_persons.province_id asc');
        $result = $this->db->get();
        $value = array();
...
//                $province_name = Conf_provinces_model::getNameById($row->province_id);
                $value [] = array(
                    ...
                    'person_fullname' => $row->person_name . ' ' . $row->person_lastname . ' ' . $row->person_secondlastname,
    ...
    public function deleteSession($person_id)
    {
        $users ['locked'] = 0;
        $this->db->where('person_id', $person_id);
        $this->db->update(self::TABLE_NAME, $users);
        self::unlock($person_id);
        $this->db->where('person_id', $person_id);
